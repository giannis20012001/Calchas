...
dataframe = pd.read_csv('../data/datasets/test_datasets/chaotic_TimeSeries_turkey_elec.csv')
dataframe.head()
plt.plot(dataframe)
autocorrelation_plot(dataframe.values[:, 0])

# AVALIAR V3 LINHAS
model00 = ARIMA(np.array(dataframe.values[:, 0]), dates=None, order=(2, 1, 0))
model11 = model00.fit(disp=1)
model11.summary()
model11.forecast()
resid9 = model11.resid
np.mean(abs(resid9)) / max(np.array(dataframe.values[:, 0]))
...
train0 = np.delete(np.array(dataframe.values[:, 0]), delete)
...
dataframe3 = pd.read_csv('../data/datasets/test_datasets/chaotic_TimeSeries_turkey_elec.csv')
real_data = np.array(dataframe3.values[:, 0][3279:3285])
